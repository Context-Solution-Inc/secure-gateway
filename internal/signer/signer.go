// Package signer mints and serializes the relay connection token (JWT) using a
// single asymmetric key, and exports the public half as PEM and JWKS.
//
// It is the token-issuing counterpart to internal/token (which only verifies).
// The Auth & License Service (cmd/auth) signs connection tokens with this
// package; the dev CLI and tests reach it through the internal/devtoken alias.
// Keeping minting here, in a package the relay never imports, preserves the
// property that the relay binary cannot mint tokens (PRD FR-3).
package signer
...
// JWKS returns a single-key JWKS document for the JWKS key source. The Auth
// service serves this at /.well-known/jwks.json; the relay fetches it via
// RELAY_JWKS_URL. The shape matches token.JWKSSource's parser.
